...
public class Result implements Render{
	...
	public void render(Request request, Response response) {
		...
		for (String key : overwrittenHeader.keySet()) 
			response.setHeader(key, overwrittenHeader.get(key));

		for (String key : dateHeader.keySet()) 
			response.setDateHeader(key, dateHeader.get(key));

		if (content != null) {
			if (contentType != null) {
				response.print(content, contentType);
			} else {
